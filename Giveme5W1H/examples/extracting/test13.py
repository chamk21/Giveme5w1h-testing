...
constraints_hyponyms = {'entity': None,'event': None, 'act': None}
...
for key,value in dat.items():
    ...
    useful_sentence = []
    for verb in value:
        before_keyword, keyword, after_keyword = sentence.partition(verb)
        after_keyword_words = after_keyword
        useful_sentence.append(after_keyword_words)
        
    longest_sentence = max(useful_sentence,key=len,default=None)
    candidates2[sentence] = longest_sentence
...
for key,value in candidates2.items():
    ...
    pos = [] 
    document = NLP(str(candidate_sentence))
    for token in document:
        pos.append(str(token.pos_))
    print(pos)
    if 'ADJ' or 'ADV' in pos and 'AUX' not in pos:
        refined_sentences[key] = candidate_sentence
    else:
        refined_sentences[key] = ''
    
for key,value in refined_sentences.items():
    print('key:',key,'\n','value:',value,'\n')
    print("==============================================================")
